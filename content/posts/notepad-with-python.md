...
After reading an excellent article by [Kazi Manzur Rashid](http://weblogs.asp.net/rashid/)on setting up a development environment for Iron Ruby using [Notepad++](http://notepad-plus-plus.org/), I was immediately struck with an idea of using the same excellent tool with Python 2.6.
 ...
 Now don’t get me wrong here, theoretically there is nothing wrong with IDLE, but having a lightweight IDE for those who don’t want to use [Pydev](http://pydev.org/)plugin for [Aptana](http://www.aptana.com/)or [Eclipse](http://www.eclipse.org/), I think Notepad++ is indeed a nice little dev tool.
 ...
 In order to Run python script from Run menu in Notepad++ you must Open Run > Run (or simply press F5), in the window that opens
  ...
  c:/PythonXX/Pythonw.exe  (If you don’t like console) is added in your Environment variables, if not you could either go to:
 ...
 You could now see your entry in Run drop down menu at the last, in case you want to edit or remove this go to Settings>Shortcut mapper and press Run Commands tab and then right click to choose options from context menu and you’re all set.
