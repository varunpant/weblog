...
```
 ...
 Running the above code will output something like below
...
```
 ...
 One can also provide another variable to add domain constraint. allowed\_domains = ["varunpant.com"] so that links which lead out of the principal domain are rejected.

 Scrapy takes care of ensuring that links which have already been visited are not visited again.
