...
IIS7 is quite modular, it is [shipped](http://blogs.iis.net/bills/archive/2008/06/02/how-iis-ships-software.aspx) with lots of goodies as separate modules and together it is now one of the most powerful and flexible web servers.

 In this post I intend to cover how we can easily configure [ARR](http://www.iis.net/downloads/microsoft/application-request-routing) and [URL Rewrite Module](http://www.microsoft.com/en-gb/download/details.aspx?id=7435) to get a similar functionality as of [Mod Proxy](http://httpd.apache.org/docs/2.2/mod/mod_proxy.html) in Apache.
 ...
 **STEP 1:** To set up a rule click on the web application from the sites tree on the left and then click on the URL Rewrite module, then from the command palette on the right hand click on Add Rule as shown in image below.   
[![add_rule](/resources/add_rule_thumb_8.png "add_rule")](/resources/add_rule_18.png)   
**STEP 2:** From the add rule dialog choose reverse proxy template   
[![reverse_proxy_template](/resources/reverse_proxy_template_12.png "reverse_proxy_template")](/resources/reverse_proxy_template_12.png)

 Configure Inbound Rules window and provide server name as shown in screen shot below

 [![](/resources/image_configureproxyurl.png)](/resources/image_2.png)
 ...
 [![image](/resources/last_page_proxy.png)](/resources/image_4.png)

 In the patterns text box entering something like ^/service(.*) will trap all requests going to <http://localdomain/service> and they will be proxied as if the requests were made like <http://anotherdomain.com/service>, The query strings if present are also carried forward, all these settings and modification are preserved in web.config of the web application, under tag. The rules after configuration look like 
