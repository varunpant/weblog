...
```
 ...
 The library comes with an [animate](http://raphaeljs.com/reference.html#Element.animate) function which is perhaps one of my favourite implementations, its simple easy and packs a big punch.

 Animation is basically a process of fiddling with the attributes of a vector in a timed routine, so lets say that we have a circle i.e
